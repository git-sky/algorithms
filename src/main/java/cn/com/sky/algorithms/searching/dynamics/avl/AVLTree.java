 ...
 * AVL树（自平衡二叉搜索树）【Hard】
 *
 * 题目：实现AVL树的插入、删除、查找操作
 *
 * 算法原理：
 * 1. 在BST基础上，要求任意节点的左右子树高度差不超过1
 * 2. 插入/删除后通过旋转恢复平衡
 * 3. 四种旋转操作：
 *    - LL型（左左）：右旋
 *    - RR型（右右）：左旋
 ...
 *
 * AVL树 vs 红黑树：
 * - AVL树：更严格的平衡，查找更快，插入/删除更慢（更多旋转）
 * - 红黑树：近似平衡，查找稍慢，插入/删除更快（更少旋转）
 *
 * 时间复杂度：
 * - 查找/插入/删除：O(log n)
 * 空间复杂度：O(n)
 *
 * 参考：《数据结构与算法分析》第4章4.4节
 * </pre>
 */
public class AVLTree<T extends Comparable<T>> {
	...
	public void display() {
		...
		if (data == null || (parent != null && leftChild == null && rightChild == null))
			return;
		System.out.println(String.format("H=%2d, %s->(%s,%s)", height, data.toString(), leftChild == null ? null : leftChild.data.toString(), rightChild == null ? null
				: rightChild.data.toString()));
		if (leftChild != null)
			leftChild.display();
		if (rightChild != null)
			rightChild.display();
	 ...
	 * */
	private static <T extends Comparable<T>> AVLTree<T> rotateLeft(AVLTree<T> tree) {
	 ...
	 * */
	private static <T extends Comparable<T>> AVLTree<T> rotateRight(AVLTree<T> tree) {
